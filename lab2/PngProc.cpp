...
// function for saving an image as PNG
size_t NPngProc::write_png_file(const char* filename, std::vector<std::vector<RGB>>& image, size_t width, size_t height) {
	FILE* fp = fopen(filename, "wb");
	if (fp == NULL)
		throw CPNGExeption("can't create png file");

	// creating and initializing png_struct
	...
	// creating and initializing png_info
	...
	// creating an array of strings to pass into png(each string is an array of RGB values)
	std::vector<png_bytep> row_pointers(height);
	for (size_t y = 0; y < height; ++y) {
		row_pointers[y] = (png_bytep)&image[y][0];  // convert std::vector<RGB> to a byte array
	}
